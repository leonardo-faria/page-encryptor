...
`scripts/generate-loader.js` is the zero-dependency Node script that does the
actual bundling and encryption. The GitHub Action ([README.md](README.md))
is a thin wrapper around it — everything here applies whether you run it
from CI or your own terminal.

```bash
node scripts/generate-loader.js ./my-site
# → my-site/loader.html
```

Open that file and the app runs. Everything — HTML, CSS, JS, images, JSON,
fonts — is inside it.
...
```bash
node scripts/generate-loader.js [dir] [options]
```
...
```bash
node scripts/generate-loader.js                              # bundle current directory
node scripts/generate-loader.js ./docs -o site.html          # explicit dir and output
node scripts/generate-loader.js . -e app/main.html           # non-standard entry
...
node /path/to/scripts/generate-loader.js .
```
...
```bash
node scripts/generate-loader.js ./site --encrypt
```
...
Also available as a [GitHub Action](README.md).
