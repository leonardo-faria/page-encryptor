...
| `encryption-key` | *(random)* | Use this exact key instead of generating a random one — see [Encryption](#encryption) |
...
AES-256-GCM. By default, a fresh random 256-bit key is generated on every
run — not a passphrase, so there's nothing to remember and nothing worth
brute-forcing offline. The key exists nowhere but this run's own output and
the ciphertext it unlocks; lose it and the bundle is unrecoverable.

Set `encryption-key` to pin the same key across builds instead — accepts a
key this action printed before (reused byte-for-byte) or any other text
(hashed into a key, with a weaker guarantee; the bundle step's log will say
so if it looks too short).

Full design rationale — why GCM, why a random key beats a passphrase by
default, and exactly what this protects against (and what it doesn't) — is
in [CLI.md](CLI.md#encryption).
...
re-running the job, which by default produces a *different* key each time —
the old bundle stays encrypted with the old one forever. If you need the
same key across runs, capture it out of a run and pass it back in via
`encryption-key`; without that input, every run generates a fresh one.
...
It does not make repeated runs deterministic in one respect, by default: the
encryption key is random every time (see [Encryption](#encryption) above).
Two runs against identical source produce different ciphertext and different
keys, unless `encryption-key` is set to pin one.
